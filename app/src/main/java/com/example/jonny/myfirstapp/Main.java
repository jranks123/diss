...
public class Main extends Activity {
   ...
   // VarTree varTree;
    ...
    Button btnFuncFinishParam;
    Button btnFuncFinishFuncCall;
  ...
  //  ArrayList<Variable> variables;
    ...
    ArrayList<String> parameterStack;
   ...
   public void hideVarButtons(){
    ...
    public void visitNode(Node tree){
        ...
        if (nodeType == Node.Type.PRINT){
         //   addToCode(code, null, R.string.print, true);  //TODO
        ...
        else if(nodeType == Node.Type.COMMA){
            code.append(", ");
        }
           ...
           /* if(((Newline)tree.parent).newlineNodeType == Newline.Type.FUNCEND){
            ...
            VarTree currentScopeVar = varRunTree.get(varRunTree.size() - 1).findTempCurVarNode(varRunTree.get(varRunTree.size() - 1));
    ...
    public void visitNodeRun(Node treeNode) {
        if (errorStack.size() == 0) {
            ...
            if (treeNode.nodeType == Node.Type.SMCLN) {
            ...
            } else if (treeNode.nodeType == Node.Type.STARTFUNC) {
                openCurlysIndent.add(true);
                ...
                //TODO: THIS IS BEING CALLED LIKE 5 TIMES WHEN IT SHOULD JUST BE CALLED ONCE
                Variable v; //TODO: make this account for varvals and functioncalls
                v = treeNode.returnDecVar(treeNode);
                Variable.Type type = v.varNodeType;
                String value = parameterStack.get(parameterStack.size() - 1);
                Variable var = new Variable(null, type, v.name, value);
                ...
                try {
                    ...
                    errorStack.add("Stack overflow due to never ending loop");
                }
    ...
    public void setCurrentScope(){
        VarTree currentVarScope = varRunTree.get(varRunTree.size() - 1).findTempCurVarNode(varRunTree.get(varRunTree.size() - 1));
        varRunTree.get(varRunTree.size() - 1).clearVarTreeIsCurrent(varRunTree.get(varRunTree.size() - 1));
    ...
    public void visitNodeVarNew(Node treeNode, Boolean fillFrom, Boolean foundCurrent) {
        Boolean addVar= false;
        if((fillFrom && foundCurrent)|| !fillFrom){
            addVar = true;
        }
            ...
            if (treeNode.isXbeforeY(treeNode, Node.Type.DEC, Node.Type.SEQ)) {
              ...
              //  if (!checkVarExists(v.name)) {
               ...
               //     showInvalidAlert("A");
                ...
                Node functionNode = treeNode.returnFunctionNode(treeNode);
                ((Function) functionNode).parameters.add(var);



        }
        else if(nodeType == Node.Type.ENDPARAMFUNCCALL){  //TODO: THIS IS BEING CALLED LIKE 5 TIMES WHEN IT SHOULD JUST BE CALLED ONCE
    ...
    public Boolean runFillVarNew(Node node,  Boolean curNodeFound,  Boolean fillFrom, Boolean fillFull, Boolean fillTo){
        if(fillFrom || fillTo) {
    ...
    public void fillVariablesArrayFull(){
        ...
        openCurlysIndent.clear();
        tree = tree.clearFunctionParams(tree);
        Boolean fillTo = false;
        Boolean fillFull = true;
        Boolean fillFrom = false;
        runFillVarNew(tree, false, fillFrom, fillFull, fillTo);
    }
                ...
                b.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View view) {
                        ...
                        if (tree.isXbeforeY(curNode, Node.Type.EVAL, Node.Type.SEQ) || tree.isXbeforeY(curNode, Node.Type.IF, Node.Type.SEQ)) {
                            ...
                            if(curNode.isXbeforeY(tree.findCurNode(tree), Node.Type.PARAMETER, Node.Type.SEQ) && openBrackets.size() == 0){
    ...
    public void onBtnClicked(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btnFuncFinishParam:
                tree = tree.addNode(tree, Node.Type.ENDPARAMFUNCCALL, "left", null);
            ...
            case R.id.btnEndFuncDec:
                ...
                if (currentNode.nodeType != Node.Type.FUNCTION) {
                    tree = tree.addNode(tree, Node.Type.ENDPARAM, "left", null);
                ...
                if (currentNodeType == Node.Type.SEQ) {
                    if (currentNd.left.nodeType == Node.Type.NEWLINE) {
                        if (((Newline) currentNd.left).newlineNodeType == Newline.Type.RETURN) {
                            if (((Return) currentNd.left.left).isInitialReturn) {
        ...
        if (currentNodeType == Node.Type.OP ){
            ...
            if (tree.isXbeforeY(currentNode, Node.Type.EVAL, Node.Type.SEQ)) {
                ...
                edtEnterString.setText("");
                //For adding parameters into function definition
                if(tree.isXbeforeY(currentNode, Node.Type.PARAMETER, Node.Type.SEQ)){
            ...
            if ((tree.returnEvalNode(currentNode)).evalNodeType == Eval.Type.NONE) {
                if ((tree.isXbeforeY(currentNode, Node.Type.PRINT, Node.Type.SEQ)) || (tree.isXbeforeY(currentNode, Node.Type.CONDITION, Node.Type.SEQ)) || (tree.isXbeforeY(currentNode, Node.Type.PARAMETER, Node.Type.SEQ))) {
...
//TODO: FIX VARIABLE SCOPE -- DONE

//TODO: having problems with for loops within functions -- DONE


//TODO: have better error reporting, so basically if there is an error, add the error message to an array list, at the end if there are any errors, output them instead of code -- DONE
